...
const chooseDate = (selector, date, isBackBtn) => {
  ...
  cy.get('.css-view-1dbjc4n > .r-overflow-1udh08x').within(() =>
    cy.contains(Number(day)).click()
  );
};

export const selectDate = (selector, date, isBackBtn) => {
  if (date !== '') {
    chooseDate(selector, date, isBackBtn);
  }
};

export const uploadFile = (fileSelector, filePath) => {
  cy.get(fileSelector).click();
  cy.get('input[type="file"]').attachFile(filePath); //hidden input[type='file] for upload elements
};
