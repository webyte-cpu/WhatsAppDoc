...
import { View, Platform, FlatList, TouchableOpacity, ScrollView } from 'react-native'
...
const DoctorInfo = ({ route, navigation }) => {
  ...
  useEffect(() => {
      const unsubscribe = navigation.addListener("focus", () => {
        navigation.setOptions({
          title: `${doctor.firstName} ${doctor.middleName[0].toUpperCase()}. ${doctor.lastName}`,
        });
      });
  ...
  const RenderMap = () => {
      return Platform.OS === 'web' 
          ? <WebMap isViewMode={true} locationCoords={selectedClinic.address.coordinates} /> 
          : <AndroidMap isViewMode={true} locationCoords={selectedClinic.address.coordinates}/>
  }
  ...
  const RenderClinics = ({clinics}) => {
      const clinicNames = ({ item: clinic }) => ( 
          ...
          >
              <View style={selectedClinic.uid === clinic.uid ? customStyle.clinicBtn: [customStyle.clinicBtn, {backgroundColor: '#f0f2f4'}]}>
                  <Text category="s2" style={{color: selectedClinic.uid === clinic.uid ? '#4a40d5' : '#c0c2c8' }}>{clinic.name}</Text>
              </View>
          </TouchableOpacity>
      )
  ...
  return (
    <ScrollView style={customStyle.contentFill}>
          ...
          <Text style={{ marginTop: 10 }}>
              {doctor.about ?? defaultSampleText}
          </Text>
          <Text category="h6" style={{ marginTop: 20 }}>
          ...
          <RenderMap />
          <Button appearance="filled" style={{marginVertical: 10}} onPress={() => console.log(selectedClinic)}>Create Appointment</Button>
      </ScrollView>
  )
};
