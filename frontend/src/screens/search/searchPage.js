...
import { getByLocationString, getByName, getNearbyDoctors } from './utils/searchDoctors';
import { DoctorsList } from './doctors/doctorsList';
import customStyle from '../../../themes/styles';

const SearchPage = ({ navigation, route }) => {
  const [ verifiedDoctors, setVerifiedDoctors ] = useState([])
  const { loading: queryLoading, error, data } = useQuery(GET_ALL_DOCTORS, {
    onCompleted: (data) => {
      setVerifiedDoctors(data.results.filter((doctor) => doctor.verificationStatus === enums.verificationStatus.VERIFIED))
    },
    pollInterval: 1000
  })
    ...
    if(verifiedDoctors.length > 0){
      ...
      switch(filter) {
        ...
        case FILTER_BY.Location: 
          if(query.query.coords) { // USE LOCATION
  ...
  return (
    <View>
      <View style={[customStyle.contentFill, {paddingBottom: 20 }]}>
        <Searchbar inputFilter={route.params.filter} inputQuery={route.params.query} route={route} inputSearch={inputSearch}/>
      </View>
      <Divider/>
